...
class RecurrentLayerWithSearch(Layer):
    ...
    def _init_params(self):
        ...
        # context gate
        if self.state.get('use_context_gate', False):
            # added by Zhaopeng Tu, 2016-01-27
            if self.state.get('use_previous_target_word_for_context_gate', False):
                # input from y
                self.GA_y = theano.shared(
                        sample_weights_classic(self.state['rank_n_approx'],
                            self.n_hids,
                            -1,
                            10 ** (-3),
                            rng=self.rng),
                        name="GA_y_%s"%self.name)
                self.params.append(self.GA_y)
            if self.state.get('use_decoding_state_for_context_gate', False):
                # input from state_before
                self.GA_h = theano.shared(
                        sample_weights_classic(self.n_hids,
                            self.n_hids,
                            -1,
                            10 ** (-3),
                            rng=self.rng),
                        name="GA_h_%s"%self.name)
                self.params.append(self.GA_h)
            if self.state.get('use_current_context_for_context_gate', False):
                # input from context
                self.GA_c = theano.shared(
                        sample_weights_classic(self.c_dim,
                            self.n_hids,
                            -1,
                            10 ** (-3),
                            rng=self.rng),
                        name="GA_c_%s"%self.name)
                self.params.append(self.GA_c)
    ...
    def step_fprop(self,
        ...
        # added by Zhaopeng Tu, 2015-01-21
        # now we compute the gating factor for source and target contexts
        if self.state.get('use_context_gate', False):
            context_gate_content = TT.zeros((target_num, self.n_hids))
            if self.state.get('use_previous_target_word_for_context_gate', False):
                context_gate_content += TT.dot(previous_word, GA_y)
            if self.state.get('use_decoding_state_for_context_gate', False):
                context_gate_content += TT.dot(state_before, GA_h)
            if self.state.get('use_current_context_for_context_gate', False):
                context_gate_content += TT.dot(ctx, GA_c)
            context_gate = self.updater_activation(context_gate_content)
            
            # added by Zhaopeng Tu, 2016-03-07
            source_context_gate = context_gate
            target_context_gate = 1. - context_gate

            # added by Zhaopeng Tu, 2016-02-23
            # the target side
            state_before *= target_context_gate
            # state below only contains contribution from target word embedding
            state_below *= target_context_gate
            reseter_below *= target_context_gate
            updater_below *= target_context_gate
            
            # the source side
            # since ctx would be returned as a result, we use another variable for the gated context
            state_below += self.c_inputer(ctx).out * source_context_gate
            reseter_below += self.c_reseter(ctx).out * source_context_gate
            updater_below += self.c_updater(ctx).out * source_context_gate
        else:
