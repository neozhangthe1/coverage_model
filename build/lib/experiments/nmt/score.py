...
def main():
    ...
    indx_word_src = pickle.load(open(state['word_indx'],'rb'))
    indx_word_trgt = pickle.load(open(state['word_indx_trgt'], 'rb'))
    ...
    if args.mode == "batch":
        ...
        for i, batch in enumerate(data_iter):
            ...
            for s in scores:
                print("{:.5e}".format(float(s)), file=score_file)
        ...
        while True:
            try:
                ...
                src_line = input('Source sequence: ')
                trgt_line = input('Target sequence: ')
